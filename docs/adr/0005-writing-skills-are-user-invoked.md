...
写作技能（`write-article`、`writing-ai-check`、`rewrite-en2zh`）与工程技能 `code-simplifier` 都设为显式调用：`SKILL.md` 加 `disable-model-invocation: true`，`agents/openai.yaml` 加 `policy.allow_implicit_invocation: false`。
